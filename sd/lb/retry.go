...
func Retry(max int, timeout time.Duration, b Balancer) endpoint.EndpointInterceptor {
...
// first.
func RetryWithCallback(timeout time.Duration, b Balancer, cb Callback) endpoint.EndpointInterceptor {
	if cb == nil {
		cb = alwaysRetry
	}
	if b == nil {
	...
	return endpoint.EndpointFunc(func(ctx context.Context, request interface{}) (response interface{}, err error) {
		...
		for i := 1; ; i++ {
			go func() {
				...
				response, err := e.Process(newctx, request)
				if err != nil {
					errs <- err
					return
				}
