...
func NewRegistrar(client Client, service Service, logger *zap.Logger) *Registrar {
	return &Registrar{
		client:  client,
		service: service,
		logger:  logger.With(zap.String("key", service.Key), zap.String("value", service.Value)),
	}
}
...
func (r *Registrar) Register() {
	if err := r.client.Register(r.service); err != nil {
		r.logger.Error("Register", zap.Error(err))
		return
	}
	if r.service.TTL != nil {
		r.logger.Info("register", zap.Int64("lease", r.client.LeaseID()))
	} else {
...
func (r *Registrar) Deregister() {
	if err := r.client.Deregister(r.service); err != nil {
		r.logger.Error("Deregister", zap.Error(err))
	} else {
		r.logger.Info("deregister")
	}
